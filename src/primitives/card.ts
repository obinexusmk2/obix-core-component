 ...
 */
export function createCard(config: CardConfig): ComponentLogicWithAccessibility {
  const {
    width = '100%',
    height = 'auto',
    loading = false,
  ...
  if (height === 'auto' && !loading) {
    console.warn(
      'Card height is "auto" which may cause Cumulative Layout Shift (CLS). Consider setting an explicit height.'
    );
  }
  ...
  const actions = {
     ...
     * Update dimensions
     */
    setDimensions(w: number | string, h: number | string) {
